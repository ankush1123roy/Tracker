...
aviobj = avifile('nl_mugII_s5.avi','compression','None');
...
[Positive,Negative] = InitializationSVM(Initialization,Initiate,Templt,x_min,y_min,width,height);
[model] = trainingClassifier(Positive,Negative);
pos_xy = [x_min,y_min];
for i = 2:M-1
Im1 = imread(a{i,1}); Im2 = imread(a{i+1,1});
x_min = pos_xy(1);y_min = pos_xy(2);
Im1 = im2double(rgb2gray(Im1)); Im2 = im2double(rgb2gray(Im2));
temp_dt = Im2 - Im1;
temp_dt= temp_dt(y_min:y_min+height-1, x_min:x_min+width-1);
[img_mono_dx,img_mono_dy] = gradient(Im1);
temp_dx = img_mono_dx(y_min:y_min+height-1, x_min:x_min+width-1);
temp_dy = img_mono_dy(y_min:y_min+height-1, x_min:x_min+width-1);
temp_dt1 = -temp_dt(:);
temp_dx1 = temp_dx(:);
temp_dy1 = temp_dy(:);
...
y_max = y_min + height-1;
   ...
   % Evaluate Jacobian
    ...
    IWarp = Im2(y_min:y_min+height-1, x_min:x_min+width-1);
    % Compute error image
    ...
    IDet = Im2(max(y_min-Offset,1):min(y_max+Offset,end), max(1,x_min-Offset):min(x_max+Offset,end));
    [accu,X,Y] = imageProcessed(IDet,blockSize,model);
    x_min= X+x_min-Offset;
    y_min=Y+y_min-Offset;
    pos_xy = [x_min,y_min];
   end
